...
function createListOfPassengers (i) {
  const passengers = [new Passenger({ id: 0, name: 'passanger number 0' })]
  for (let j = 1; j < i; j++) {
...
refactorPassenger(4, 'Tanya', passengers)
//console.log(passengers)
...
passengers = deletePassenger(12, passengers)
//console.log(passengers)
...
//console.log(findPassengerById(4, passengers));

cities = ['Lviv', 'Kyiv', 'Kharkiv', 'Odessa', 'Dnipro', 'Donetsk', 'Mykolaiv', 'Chernivtsi']

function createListOfTrains (i) {
  const trains = [new Train({ id: 0, direction: 'lviv - odessa', vagons: 12 })]
  for (let j = 1; j < i; j++) {
...
addNewTrain('lviv - Uzhgorod', 2, trains)
//console.log(trains);
...
byeTicket(passengers[20], trains[12], tickets, soldTickets)
//console.log(soldTickets);
...
function NumberOfMostUsefullTrains (soldTickets) {
  ...
  for (let i = 0; i < soldTickets.length; i++) {
    if (myMap.has(soldTickets[i].Train) === false) { myMap.set(soldTickets[i].Train, 1) } else {
